...
const CONTROLLED_AUDIO_WRAPPER_SUFFIXES = new Set([
  AUDIO_WRAPPER_SUFFIX,
  AUDIO_FORMAT_SUFFIX,
  AUDIO_RUNTIME_SUFFIX,
  TONE_WRAPPER_SUFFIX,
])
...
const BLOCKED_TONE_CONTEXT_EXPORTS = new Set([
  'getContext',
  'setContext',
  'context',
  'Context',
...
export function blockAudioApisPlugin(): Plugin {
  return {
    ...
    transform(code, id) {
      ...
      if (
        Array.from(CONTROLLED_AUDIO_WRAPPER_SUFFIXES).some((suffix) =>
      ...
      const isBlockedConstructorExpression = (node: NodeAny): boolean => {
        ...
        if (target?.type === 'Identifier') {
          return (
            BLOCKED_AUDIO_CONSTRUCTORS.has(target.name) ||
            blockedConstructorAliases.has(target.name)
          )
        }
        ...
        if (target?.type === 'MemberExpression') {
          ...
          return Boolean(
            objectName &&
            propertyName &&
            GLOBAL_OBJECTS.has(objectName) &&
            BLOCKED_AUDIO_CONSTRUCTORS.has(propertyName),
          )
        }
      ...
      const isToneNamespaceMember = (node: NodeAny, blockedMembers: Set<string>): boolean => {
        const chain = extractMemberChain(node)
        return Boolean(
          chain && toneNamespaceIdentifiers.has(chain[0]) && blockedMembers.has(chain[1]),
        )
      }
      ...
      const isBlockedToneContextSource = (node: NodeAny): boolean => {
        ...
        if (target?.type === 'MemberExpression') {
          ...
          return Boolean(
            ...
            (BLOCKED_TONE_CONTEXT_EXPORTS.has(chain[1]) ||
              chain.includes('rawContext') ||
              chain.includes('context')),
          )
        }
      ...
      const isToneDestinationSource = (node: NodeAny): boolean => {
        ...
        if (target?.type === 'MemberExpression') {
          ...
          return Boolean(
            chain &&
            toneNamespaceIdentifiers.has(chain[0]) &&
            (chain[1] === 'Destination' || chain[1] === 'Master'),
          )
        }
      ...
      walk(ast, (node) => {
        if (node.type === 'AssignmentExpression') {
          ...
          if (
            ...
            getMemberPropName(left) === 'mute' &&
            isToneDestinationSource(left.object)
          ) {
            ...
            addError({
              ...
              message:
                '禁止业务代码直接修改 Tone.js 总输出静音状态，静音必须由宿主 window.app.mute/unmute 统一管控。',
        ...
        if (node.type === 'CallExpression') {
          ...
          if (
            ...
            blockedToneContextIdentifiers.has(node.callee.name)
          ) {
            const start = node.start ?? 0
            const end = node.end ?? start + 20
            addError({
          ...
          if (node.callee?.type === 'MemberExpression') {
            ...
            if (
              methodName &&
              BLOCKED_AUDIO_METHODS.has(methodName) &&
              isBlockedInstanceSource(objectExpr)
            ) {
              const start = node.start ?? 0
              const end = node.end ?? start + 20
              addError({
        ...
        if (node.type === 'MemberExpression') {
          ...
          if (
            isToneNamespaceMember(node, BLOCKED_TONE_CONTEXT_EXPORTS) ||
            (propertyName === 'rawContext' && isBlockedToneContextSource(node.object))
          ) {
            const start = node.start ?? 0
            const end = node.end ?? start + 20
            addError({
              ...
              message:
                '禁止访问 Tone.js 底层 AudioContext/rawContext，请使用 @/lib/tone 的高层音频能力。',
              codeSnippet: getCodeSnippet(code, start, end),
            })
          }
