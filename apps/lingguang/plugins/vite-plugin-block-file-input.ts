...
function isDocumentReference(
  ...
  return (
    ...
    (root.text === 'window' || root.text === 'globalThis' || root.text === 'self')
  )
}
...
function isInputConstructorCall(
  ...
  return (
    ...
    isHtmlInputConstructorReference(current.expression, stringBindings)
  )
}
  ...
  const visit = (node: ts.Node) => {
    if (ts.isJsxSelfClosingElement(node) || ts.isJsxOpeningElement(node)) {
      if (getJsxTagName(node) === 'input') {
        ...
        const typeValue = normalizeValue(
          getJsxStaticStringValue(typeAttribute?.initializer, stringBindings),
        )
        if (typeValue === 'file') {
          report(
            node,
    ...
    if (ts.isBinaryExpression(node) && node.operatorToken.kind === ts.SyntaxKind.EqualsToken) {
      ...
      if (
        ...
        member.name === 'type' &&
        rightValue === 'file' &&
        isInputReference(member.target)
      ) {
        report(
          node,
