...
export function asapBuildContractPlugin(): Plugin {
  ...
  function getModuleGraphRecord(
    id: string,
    getModuleInfo: (moduleId: string) => ModuleGraphInfo | null,
  ) {
    ...
    return {
      ...
      dynamicallyImportedIds: sortUnique(
        (info.dynamicallyImportedIds ?? []).map((item) => relativeId(item)),
      ),
  ...
  return {
    ...
    handleHotUpdate(ctx) {
      ...
      const affectedModules = ctx.modules
        .map((mod) => relativeId(mod.id ?? mod.url))
        .sort((a, b) => a.localeCompare(b))
      server?.ws.send({
        type: 'custom',
    ...
    generateBundle(_options, bundle) {
      ...
      for (const item of Object.values(bundle)) {
        ...
        const source =
          typeof item.source === 'string' ? item.source : (item.source ?? new Uint8Array())
        assets.push({
          fileName: item.fileName,
          name: item.name ?? null,
      ...
      const moduleGraph = Array.from(new Set([...outputModuleIds, ...moduleInfoIds]))
        .sort((a, b) => relativeId(a).localeCompare(relativeId(b)))
        .map((id) => getModuleGraphRecord(id, this.getModuleInfo.bind(this)))
