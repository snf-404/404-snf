...
const DEV_MOCK_IMAGE_URL =
  'https://gw.alipayobjects.com/render/p/yuyan/180023180034745799/assets/image.jpg'
...
const installLocalStorageBridge = () => {
  ...
  try {
    ...
    console.log(
      '[FLASH APP] 安装 window.localStorage bridge 失败，使用 __bridgeLocalStorage 兜底',
      error,
    )
  }
  try {
...
window.addEventListener('message', (event) => {
  ...
  if (eventData.type === 'API_STREAM_ERROR') {
    ...
    try {
      ;(_a = callbackInfo.onError) == null
        ? void 0
        : _a.call(callbackInfo, { message: error, code })
    } catch (e) {
  ...
  switch (action) {
    ...
    case 'lingguang.db.execute':
      return {
        success: false,
        data: { rowsAffected: 0 },
        message: 'db.execute not available in dev mode',
      }
    ...
    if (messageData.type === 'CANDYJAR_EVENT') {
      ...
      if (eventType === 'CandyJar.WebContainer.VisibilityChange') {
        ...
        if (visibilityState === 'hidden') {
          void ((_a = avGuard == null ? void 0 : avGuard.pauseAll) == null
            ? void 0
            : _a.call(avGuard))
        } else if (visibilityState === 'visible') {
          void ((_b = avGuard == null ? void 0 : avGuard.resumeAll) == null
            ? void 0
            : _b.call(avGuard))
        }
      }
      ...
      if (eventData.isFinal !== void 0)
        customEvent.isFinal = eventData.isFinal === '1' || eventData.isFinal === true
...
;(function () {
  ...
  const getAppContainer = () => {
    return (
      ...
      document.getElementById('full-component')
    )
  }
        ...
        waitForAMapLoader()
          ...
          .catch((e) => {
            ...
            reject(
              e instanceof Error
                ? e
                : new Error(getErrorMessage(e, '[FLASH APP] AMapLoader 初始化失败')),
            )
          })
      }
      script.onerror = () => {
...
;(function () {
  ...
  async function logClick(element, _event) {
    ...
    try {
      await window.lingguang.monitor
        .event({
